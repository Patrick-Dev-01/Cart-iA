...
export class GeminiLlmService extends LlmService{
    suggestCarts(relevantProductsByStore: { store_id: number; products: { id: number; name: string; price: number; similarity: number; }[]; }[], input: string): Promise<(SuggestCarts & { responseId: string; }) | null> {
    ...
    answerMessage(message: string, previousMessageId: string | null): Promise<(AnswerMessage & { responseId: string; }) | null> {
        throw new Error("Method not implemented.");
    }
