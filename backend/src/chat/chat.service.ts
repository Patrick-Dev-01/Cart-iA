...
export class ChatService{
    ...
    async getChatSession(sessionId: number){
        ...
        return {
            ...result.rows[0],
            messages: populatedMessages.sort((a, b) => 
    ...
    private async populateMessages(
        ...
        return Promise.all(messages.map(async (message) => {
            ...
            const cartsResult = await this.postgresService.client.query<{
                ...
                GROUP BY c.id, c.store_id, s.name, c.score
            `, [message.id]
            );
            ...
            return  {
                ...
                carts: cartsResult.rows.map(row => ({
                    ...
                    score: row.score,
                    total: row.products.reduce((sum, product) => sum + product.price + product.quantity, 0)
                }))
            }
        }))
    }
    ...
    async confirmAction(sessionId: number, actionId: number){
        ...
        if(result.rows[0].action_type === 'suggest_carts'){
            ...
            if(relevantProductsGroupedByStore.rows.length === 0){
                throw new NotFoundException('No relevant products found for the given input');
            }

            const llmResponse = await this.llmService.suggestCarts(relevantProductsGroupedByStore.rows,
                result.rows[0].payload.input
            )

            if(!llmResponse || !llmResponse.carts){
                throw new BadGatewayException('Failed to get cart suggestions from LLM');
            }
            ...
            await this.saveSuggestCarts(message.id, llmResponse.carts);
        }else{
            throw new InternalServerErrorException(`Action type ${result.rows[0].action_type} is not supported`);
        }
    }

    private async saveSuggestCarts(messageId: number, carts: {
        store_id: number;
        score: number;
        products: {
            id: number;
        ...
        for (const cart of carts) {
            const cartResult = await this.postgresService.client.query(
                `INSERT INTO carts (user_id, store_id, score, suggested_by_message_id, active) VALUES ($1, $2, $3, $4, FALSE) RETURNING id`,
    ...
    async chooseCart(cartId: number, user_id: number){
