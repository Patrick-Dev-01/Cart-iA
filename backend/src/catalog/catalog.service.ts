...
export class CatalogService implements OnApplicationBootstrap {
  ...
  async handleEmbeddingWebhook(rawBody: string, headers: Record<string, string>) {
    ...
    if(!results || results.length === 0){
      console.warn('No results from LLM embedding webhook');
      return;
    }
    ...
    for (const result of results){
      ...
      console.log(`Updated product ${productId} with new embedding successfully`);
    }
  }

  async getCatalog(search = '') {
    let query = `
      SELECT products.id, products.name, products.price, products.embedding, json_build_object('id', stores.id, 'name', stores.name) 
      AS store FROM products JOIN stores ON products.store_id = stores.id`;

    if(search){
      query += ` WHERE products.name ILIKE $1`;
    }

    const result = await this.postgresService.client.query<Product>(query, search ? [`%${search}%`] : undefined);
