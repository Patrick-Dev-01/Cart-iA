...
describe('Cart (e2e)', () => {
  ...
  it('should add product to the cart', async () => {
    const response = await request(app.getHttpServer()).post('/cart')
    .send({
        productId: 1,
        quantity: 2
    });
  ...
  it('should add a product to an existing cart if the store is the same', async () => {
    ...
    const response2 = await request(app.getHttpServer()).post('/cart')
    .send({
        productId: 2,
        quantity: 3
    });
    ...
    const response3 = await request(app.getHttpServer()).post('/cart')
    .send({
        productId: 2,
        quantity: 1
    });
  ...
  it('should remove a product from the cart if quantity is 0', async () => {
    ...
    const response2 = await request(app.getHttpServer()).delete(`/cart/${response.body.id}/items/1`).send({
      quantity: 0,
    });
